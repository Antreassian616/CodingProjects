...
for paragraph in paragraphs:
    ...
    generated_pairs.append({"ID": paragraph.pop('code'), "System Prompt": system_prompt, "question": question, "answer": answer, "category": category, "Flagged": flagged, "Is Duplicate": is_duplicate })

    with open('generated_pairs_with_cosine_similarity_test.json', 'w') as output_file:
...
similarity_threshold = 0.5  # Threshold for considering questions as similar
...
for i in range(len(similarities)):
    for j in range(i + 1, len(similarities)):  # Avoid repeating pairs and self-comparison
        if similarities[i, j] > similarity_threshold:
            ...
            with open('flagged_prompts.json', 'w') as output_file:
